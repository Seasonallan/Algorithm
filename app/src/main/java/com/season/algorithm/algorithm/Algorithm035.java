...
public class Algorithm035 extends AlgorithmLog {
    ...
    public String getDesc() {
        return "在数组中的两个数字，如果前面一个数字大于后面的数字，则这两个数字组成一个逆序对。";
    }
     ...
     */
    public int mergeSort(Integer[] array, Integer[] temp, int left, int right) {
        //递归终止条件：
     ...
     * 融合左右子序列
     */
    public int merge(Integer[] array, Integer[] temp, int left, int mid, int right) {
        if (left > right) return 0;
